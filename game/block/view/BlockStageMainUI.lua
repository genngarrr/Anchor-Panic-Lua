-- @FileName:   BlockStageMainUI.lua
-- @Description:   文件描述
-- @Author: ZDH
-- @Date:   2023-07-21 10:59:29
-- @Copyright:   (LY) 2023 雷焰网络

module('game.block.view.BlockStageMainUI', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("block/BlockStageMainUI.prefab")
...
function close(self)
    GameDispatcher:dispatchEvent(EventName.CLOSE_BLOCK_DUPPANEL)

    super.close(self)
end

function addAllEventListener(self)
    GameDispatcher:addEventListener(EventName.ONCLOSE_BLOCK_DUPPANEL, self.onDupInfoPanelClose, self)
    GameDispatcher:addEventListener(EventName.OPEN_BLOCK_SCENEUI, self.close, self)
    GameDispatcher:addEventListener(EventName.BLOCK_RECEIVE_REWARD, self.onDataRefresh, self)

end

function removeAllEventListener(self)
    GameDispatcher:removeEventListener(EventName.ONCLOSE_BLOCK_DUPPANEL, self.onDupInfoPanelClose, self)
    GameDispatcher:removeEventListener(EventName.OPEN_BLOCK_SCENEUI, self.close, self)
    GameDispatcher:removeEventListener(EventName.BLOCK_RECEIVE_REWARD, self.onDataRefresh, self)

end

function onClickTeaching(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BLOCK_TEACHINGVIEW)
end

function onClickReward(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BLOCK_REWARDVIEW)
end

function onClickRank(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BLOCK_RANKPANEL)
end
...
function onSelectMapTab(self, area_id)
    local areaConfigDic = block.BlockManager:getAreaConfigDic()
    ...
    for i = 1, length do
        ...
        local dupItem = SimpleInsItem:create(self.mDupItem, self.list, "block_stageMainUI_dupItem")
        self.m_dupItemList[dup_id] = dupItem

        local configVo = block.BlockManager:getDupConfigVo(dup_id)
        ...
        if Odd then
            ...
            mRayPoint.anchoredPosition = gs.Vector2(mRayPoint.anchoredPosition.x, -74)
        end

        local isPass = block.BlockManager:isPassDup(dup_id)
        dupItem:getChildGO("mImgPass"):SetActive(isPass)

        local isTimeOpen = configVo:isOpen()
        local lastPass = true
        if configVo.pre_id ~= 0 then
            lastPass = block.BlockManager:isPassDup(configVo.pre_id)
        end
        ...
        dupItem:addUIEvent("mRayPoint", function ()
            ...
            StorageUtil:saveBool1(gstor.BLOCK_DUPNEWOPENSTR .. dup_id, true)
            self:refreshDupRedPoint()
...
function onSelectDupItem(self, dupConfigVo)
    ...
    GameDispatcher:dispatchEvent(EventName.OPEN_BLOCK_DUPPANEL, dupConfigVo)
end
...
function refreshDupRedPoint(self)
    for i = 1, #self.m_areaConfig.stage_list do
        ...
        if dupItem then
            if block.BlockManager:getDupShowRed(block.BlockManager:getDupConfigVo(dup_id)) then
                RedPointManager:add(dupItem:getChildTrans("mRayPoint"), nil, 116.5, 25.5)
            else
...
function refreshStarAwardRedPoint(self)
    if block.BlockManager:getStarRewardRedState() then
