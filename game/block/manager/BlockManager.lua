-- @FileName:   BlockManager.lua
-- @Description:   文件描述
-- @Author: ZDH
-- @Date:   2023-11-28 15:28:41
-- @Copyright:   (LY) 2023 雷焰网络

module('game.block.manager.BlockManager', Class.impl(Manager))
...
function parseAreaConfigData(self)
    ...
    local baseData = RefMgr:getData("block_area_data")
    for key, data in pairs(baseData) do
        local baseVo = block.BlockAreaConfigVo.new()
...
function parseDupConfigVo(self)
    if not self.m_dupConfigVoDic then
        ...
        local baseData = RefMgr:getData("block_data")
        for key, data in pairs(baseData) do
            local baseVo = block.BlockDupConfigVo.new()
            baseVo:parseCogfigData(key, data)
...
function parseGridConfigVo(self)
    if not self.m_gridConfigVoDic then
        self.m_gridConfigVoDic = {}
        self.m_gridWeight = 0
        local baseData = RefMgr:getData("block_icon_data")
        for i = 1, #baseData do
            local data = baseData[i]
            self.m_gridWeight = self.m_gridWeight + data.weight
            local baseVo = block.BlockGridConfigVo.new()
            baseVo:parseCogfigData(i, data, self.m_gridWeight)
            self.m_gridConfigVoDic[i] = baseVo
        end
    end

    return self.m_gridConfigVoDic[map_id]
end

function getGridConfigList(self)
    if not self.m_gridConfigVoDic then
        self:parseGridConfigVo()
    end

    return self.m_gridConfigVoDic, self.m_gridWeight
end

function parseRewardkConfigVo(self)
    if not self.m_rewardConfigVoDic then
        self.m_rewardConfigVoDic = {}
        local baseData = RefMgr:getData("block_task_data")
        for key, data in pairs(baseData) do
            local baseVo = block.BlockRewardConfigVo.new()
            baseVo:parseCogfigData(key, data)
...
function getStarRewardRedState(self)
    local score = block.BlockManager:getAllScore()
...
function getDupShowRed(self, dupConfigVo)
    ...
    if self:isPassDup(lastDup_id) and not self:isPassDup(dupConfigVo.id) and dupConfigVo:isOpen() and not StorageUtil:getBool1(gstor.BLOCK_DUPNEWOPENSTR .. dupConfigVo.id) then
        return true
    end
