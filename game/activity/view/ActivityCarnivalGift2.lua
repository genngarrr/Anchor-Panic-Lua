--[[ SetActive
-----------------------------------------------------
@filename       : ActivityCarnivalGift2
@Description      狂欢好礼
@date           : 2024-09-10 
@Author         : Shuai
...
module('game.activity.view.ActivityCarnivalGift2', Class.impl(TabSubView))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("activity/ActivityCarnivalGift2.prefab")
...
function active(self)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_CARNIVAL_GIFT_INFO2, self.updateView, self)
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_CARNIVAL_GIFT_INFO2, self.updateView, self)
    self:clearAllItem()
...
function initViewText(self)
    self.mTxtTitle.text = _TT(138509)
    self.mTxtTitle_2.text = _TT(138510)
    self.mTxtTitle_3.text = _TT(98107) 
    self.mTxtTitle_4.text = _TT(138511)
    local rechargeVo = recharge.RechargeManager:getRechargeVoByDetail(recharge.RechargeType.CARNIVAL_GIFT2)
    local priceStr = _TT(138512, rechargeVo.RMB)
...
function updateView(self,isFirst)
    local dic = sysParam.SysParamManager:getValue(SysParamType.ACTIVITY_CARIVAL_GIFT_SHOW_HERO_INFO2)
   ...
   self:clearAllItem()
   self:addTime()
   self.mBtnRecharge:SetActive(not activity.ActitvityExtraManager:getIsCarnivalRecharge2()) 
   local list = activity.ActitvityExtraManager:getCarnivalGiftConfigList2()
   for i, vo in ipairs(list) do
        ...
        numItem:addUIEvent("mCanRec",function ()
            if not activity.ActitvityExtraManager:getIsCarnivalRecharge2() then
                gs.Message.Show(_TT(138509))
                return
            end
            GameDispatcher:dispatchEvent(EventName.REQ_REC_CARNIVAL_GIFT_AWARD2,vo:getId())
        end)
        ...
        numItem:getChildGO("mImgDay"):GetComponent(ty.AutoRefImage):SetImg(UrlManager:getPackPath("activity/Carnival2_"..vo:getId()..".png"),true)
...
function updateTime(self)
    if activity.ActivityManager:getActivityVoById(activity.ActivityId.Activity_Carnival_Gift2) then
        local RemainingTime =activity.ActivityManager:getActivityVoById(activity.ActivityId.Activity_Carnival_Gift2):getEndTime() - GameManager:getClientTime()
...
function onClickHandle(self)
    if not activity.ActitvityExtraManager:getIsCarnivalRecharge2() then--CARNIVAL_GIFT2
        recharge.sendRecharge(recharge.RechargeType.CARNIVAL_GIFT2, nil, "1")
    end
end
