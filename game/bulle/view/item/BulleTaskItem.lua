--[[ 
-----------------------------------------------------
@filename       : BulleTaskItem
@Description    : 活动玩法任务Item
@date           : 2023/5/24 15:00
@Author         : Tonn
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------]]
module("bulle.BulleTaskItem ", Class.impl("lib.component.BaseItemRender"))
...
function setData(self, param)
    ...
    if self.data then
        ...
        local received = bulle.BulleManager:checkTaskAwardReceived(self.data.id)
        self:getChildGO("mGroupReceived"):SetActive(received)

        local taskMsgVo =  bulle.BulleManager:getTaskMsgVo(self.data.id)
