--[[ 
-----------------------------------------------------
@filename       : ArenaLogAttackTabView
@Description    : 巅峰竞技场进攻界面
@copyright      : (LY) 2020 雷焰网络
-----------------------------------------------------
]]
module("arenaEntrance.ArenaLogAttackTabView", Class.impl(TabSubView))
...
function active(self)
    super.active(self)
    self.m_childGos["mBtnClickAll"]:SetActive(false)
    self.charIndex = 0
    self.charIndex2 = 20
    MoneyManager:setMoneyTidList({})
    self.m_childGos["Content"]:GetComponent(ty.RectTransform).localPosition = gs.VEC3_ZERO
    GameDispatcher:dispatchEvent(EventName.REQ_ARENA_HELL_LOG)
    GameDispatcher:addEventListener(EventName.UPDATE_ARENA_HELL_LOG, self.updateView, self)

    arenaEntrance.ArenaEntranceManager.IsSelfAttack = self.getOpenType() == arenaEntrance.ArenaEntaceLogType.TypeAttack
    --self:updateView()
end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_ARENA_HELL_LOG, self.updateView, self)
...
function updateView(self, tabType)
    local len = 1
    self:closeAllOpenSubPanel()
    local list = arenaEntrance.ArenaEntranceManager:getArenaLogList()
    local defenseList = {}
    if list then
    ...
    local isEmpty = #list < 1 or
    (#defenseList < 1 and self.getOpenType() == arenaEntrance.ArenaEntaceLogType.TypeDefense) or
    list == nil or (#list == #defenseList and self.getOpenType() == arenaEntrance.ArenaEntaceLogType.TypeAttack)
    ...
    for idx, battleVo in pairs(list) do
        ...
        if battleVo.isSelfAttack == (self.getOpenType() == arenaEntrance.ArenaEntaceLogType.TypeAttack) then
            --筛选对应页签相应的数据  
            local item = SimpleInsItem:create(self.m_childGos["ArenaLogItem"], self.m_childGos["Content"].transform, "ArenaLogAttackTabViewmArenaHellLogItem")
            item.defIndex = 1
            ...
            local function updateTeam()
                ...
                if self.getOpenType() == arenaEntrance.ArenaEntaceLogType.TypeAttack then
                    item:getChildGO("mImgWin1"):SetActive(battleVo.teamList[item.defIndex].result == 1)
                    item:getChildGO("mImgWin2"):SetActive(battleVo.teamList[item.defIndex].result == 2)
                else
                    item:getChildGO("mImgWin1"):SetActive(battleVo.teamList[item.defIndex].result == 2)
                    item:getChildGO("mImgWin2"):SetActive(battleVo.teamList[item.defIndex].result == 1)
                end
                ...
                local formationHero
                local formationTid = battleVo.teamList[item.defIndex].self_formation_tid
                if formationTid > 0 then
                    local formationVo = formation.FormationManager:getFormationConfigVo(formationTid)
                    if formationVo then
                        formationHero = formationVo:getFormationList()
                    else
                        logError("编队信息配置错误，后端数据  编队Id：" .. formationTid .. "  在前端配置表查询不到")
                    end
                end
                local hasHero = {}
                local selfPos = battleVo.mTeamPosData[item.defIndex].my.posList
                for posIdx, posVo in pairs(selfPos) do
                ...
                if formationHero then
                    for _, Vo in pairs(formationHero) do
                        ...
                        if not table.indexof(hasHero, teamPos) then
                            local emptyImg = SimpleInsItem:create(self.mEmptyHero, item:getChildTrans("mImgL" .. teamPos), "ArenaLogAttackTabViewmArenaLogEmptyHero")
                ...
                hasHero = nil
                local hasMonster = {}
                local enemyPos = battleVo.mTeamPosData[item.defIndex].enemy.posList
                for enemyPosIdx, posVo in pairs(enemyPos) do
                ...
                local formationMons
                local enemyTid = battleVo.teamList[item.defIndex].enemy_formation_tid
                if enemyTid > 0 then
                    if battleVo.isRobat then
                        formationMons = formation.FormationManager:getMonFormationConfigVo(enemyTid).m_formationList
                    else
                        formationMons = formation.FormationManager:getFormationConfigVo(enemyTid).m_formationList
                    end
                end
                --  
                if formationMons then
                    for _, Vo in pairs(formationMons) do
                        ...
                        if not table.indexof(hasMonster, teamPos) then
                            local emptyImg = SimpleInsItem:create(self.mEmptyHero, item:getChildTrans("mImgR" .. teamPos), "ArenaLogAttackTabViewmArenaLogEmptyHero")
                            gs.TransQuick:UIPos(emptyImg:getGo():GetComponent(ty.RectTransform), 0, 0)
                            table.insert(m_teamDic_list, emptyImg)
                        end
                    end
            ...
            local function showTeamClick()
                if not battleVo.teamList[item.defIndex].can_replay then gs.Message.Show(_TT(62226)) return end
                if self.mTeamDic[idx] == nil then
                    self.mTeamDic[idx] = {}
                end
            ...
            item:addUIEvent("mBtnShowTeam", showTeamClick)

            local function updateBtnRound()
                for i = 1, #item.roundList do
                    item.roundList[i].transform:Find("mSelect").gameObject:SetActive(i == item.defIndex)
                end
            end

            local function selectRoundChange(index)
                item.defIndex = index
                self:recoverSingleTeam(idx)
                updateTeam()
                updateBtnRound()
            ...
            item:addUIEvent("mBtnRound1", function()
                selectRoundChange(1)
            end)
            item:addUIEvent("mBtnRound2", function()
                selectRoundChange(2)
            end)
            item:addUIEvent("mBtnRound3", function()
                selectRoundChange(3)
            end)

            item.roundList = {}
            table.insert(item.roundList, item:getChildGO("mBtnRound1"))
            table.insert(item.roundList, item:getChildGO("mBtnRound2"))
            table.insert(item.roundList, item:getChildGO("mBtnRound3"))
            updateBtnRound(item.defIndex)
            item:addUIEvent("mBtnReplay", function()
                if not battleVo.teamList[item.defIndex].can_replay then gs.Message.Show(_TT(198)) return end

                -- '是否查看本场斗录像？'
                UIFactory:alertMessge(_TT(178), true, function()
                    if battleVo.battleId ~= 0 then
                        arenaEntrance.ArenaEntranceManager.roundTime = item.defIndex
                        fight.FightManager:reqReplay(PreFightBattleType.Arena_Peak_Pvp, battleVo.battleId, item.defIndex)
                    else
            ...
            item:addUIEvent("mBtnFightData", function()
                --TODO
                if not battleVo.teamList[item.defIndex].can_replay then gs.Message.Show(_TT(62225)) return end
                fight.FightController:reqBattleRePlayDate(PreFightBattleType.Arena_Peak_Pvp, battleVo.battleId)
            end)
            ...
            item:getChildGO("mTxtRoundN1"):GetComponent(ty.Text).text = _TT(62263).."1"
            item:getChildGO("mTxtRoundS1"):GetComponent(ty.Text).text = _TT(62263).."1"
            item:getChildGO("mTxtRoundN2"):GetComponent(ty.Text).text = _TT(62263).."2"
            item:getChildGO("mTxtRoundS2"):GetComponent(ty.Text).text = _TT(62263).."2"
            item:getChildGO("mTxtRoundN3"):GetComponent(ty.Text).text = _TT(62263).."3"
            item:getChildGO("mTxtRoundS3"):GetComponent(ty.Text).text = _TT(62263).."3"
            ...
            --item:getChildGO("mImgEnemyRank"):GetComponent(ty.AutoRefImage):SetImg(arena.ArenaManager:getSegmentVo(battleVo.segment):getRankIcon(), false)
            --item:getChildGO("mImgRankPlayer"):GetComponent(ty.AutoRefImage):SetImg(arena.ArenaManager:getSegmentVo(battleVo.enemySegment):getRankIcon(), false)
            ...
            if battleVo.isRobat then
                local robotVo = arenaEntrance.ArenaEntranceManager:getRobotData(battleVo.playerId)
    ...
    if self.mClickShowAll then
        ...
        for idx, battleVo in pairs(list) do
            if battleVo.isSelfAttack == (self.getOpenType() == arena.TabViewType.TypeAttack) then
                ...
                local hasHero = {}
                local selfPos = battleVo.mTeamPosData[item.defIndex].my.posList
                for heroTid, posVo in pairs(selfPos)
                do
                    local teamPos = posVo.y * 4 - (posVo.x - 1)
                    local grid = HeroHeadGrid:poolGet()
                    grid:setData(hero.HeroManager:getHeroVoByTid(posVo.heroTid))
                ...
                local hasMonster = {}
                local enemyPos = battleVo.mTeamPosData[item.defIndex].enemy.posList
                for heroTid, posVo in pairs(enemyPos)
                do
                    local teamPos = (posVo.y - 1) * 4 + posVo.x
                    local grid = HeroHeadGrid:poolGet()
                    ---要改
                    ...
                    if enemyTid > 0 then
                        if battleVo.isRobat then
                            local formationVo = formation.FormationManager:getMonFormationConfigVo(enemyTid)
                            if formationVo then
                                formationMons = formationVo:getFormationList()
                            else
                                logError("敌方怪物编队信息配置错误，后端数据  编队Id：" .. enemyTid .. "  在前端配置表查询不到")
                            end
                        else
                            local formationVo = formation.FormationManager:getFormationConfigVo(enemyTid)
                            if formationVo then
                                formationMons = formationVo:getFormationList()
                            else
                                logError("敌方编队信息配置错误，后端数据  编队Id：" .. enemyTid .. "  在前端配置表查询不到")
                            end
                        end
                    end
                    --
...
function getOpenType()
    return arenaEntrance.ArenaEntaceLogType.TypeAttack
end
