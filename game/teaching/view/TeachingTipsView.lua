--[[ 
-----------------------------------------------------
@filename       : TeachingTipsView
@Description    : 上阵教学提示面板
@date           : 2021-09-06 14:54:05
@Author         : Jacob
@copyright      : (LY) 2021 雷焰网络
-----------------------------------------------------
]]
module('game.teaching.view.TeachingTipsView', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("teaching/TeachingTipsView.prefab")
...
function active(self, args)
    ...
    self.mTxtTitle.text = _TT(self.title)
    self.mTxtContent.text = _TT(self.des)
    -- self:updateView()
...
function updateView(self)
    ...
    for i, resName in ipairs(self.resList) do
        ...
        item:getChildGO("mImgInfo"):GetComponent(ty.AutoRefImage):SetImg(UrlManager:getBgPath(string.format("teaching/%s.jpg", resName)), true)
...
function setPosList(self)
    ...
    self.frameId = LoopManager:addFrame(3, 1, self, function()
        ...
        for i = 1, self.mScrollContent.childCount do
            ...
            local posx = (self.mScrollView.rect.width - 410) * (i - 1) / horizontalLrngth
