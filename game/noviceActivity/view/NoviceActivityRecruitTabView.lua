--[[ 
-----------------------------------------------------
@filename       : NoviceActivityRecruitTabView
@Description    : 新手活动 招募次数界面
@date           : 2023-6-6 16:50:35
@Author         : tonn
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------
]]
module("noviceActivity.NoviceActivityRecruitTabView", Class.impl(TabSubView))
UIRes = UrlManager:getUIPrefabPath("noviceActivity/NoviceActivityRecruitTabView.prefab")
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_RECEIVE_RECRUIT, self.setScollerData, self)
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_RECEIVE_RECRUIT, self.setScollerData, self)
    MoneyManager:setMoneyTidList()
...
function getItemRender(self)
    return noviceActivity.NoviceActivityRecruitItem
end

function setScollerData(self)
    if not self.list then
        self.list = table.copy(noviceActivity.NoviceActivityManager:getRecuitConfig())
    end

    local msgList = noviceActivity.NoviceActivityManager.mActivityRecruitMsgDic
    if msgList and next(msgList) then
        local listHelper = {}
        for _, id in pairs(msgList) do
            local vo = noviceActivity.NoviceActivityManager:getRecuitItemConfigById(id)
