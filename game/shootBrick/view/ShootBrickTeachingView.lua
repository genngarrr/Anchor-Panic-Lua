-- @FileName:   ShootBrickTeachingView.lua
-- @Description:   描述
-- @Author: ZDH
-- @Date:   2025-01-15 14:47:25
-- @Copyright:   (LY) 2024 锚点降临

module('game.shootBrick.view.ShootBrickTeachingView', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("shootBrick/ShootBrickTeachingView.prefab")
...
function active(self, args)
    ...
    self.resList = {"shootBrick_teaching01.png"}
    self.des = {138703}
    self.closeCall = args.closeCall
...
function updateView(self)
    ...
    for i, resName in ipairs(self.resList) do
        ...
        item:getChildGO("mImgInfo"):GetComponent(ty.AutoRefImage):SetImg(UrlManager:getBgPath(string.format("shootBrick/%s", resName)), true)
...
function setPageIndex(self, index)
    ...
    if table.nums(self.mItemList) <= 1 then
