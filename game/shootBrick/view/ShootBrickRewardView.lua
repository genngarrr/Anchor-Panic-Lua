-- @FileName:   ShootBrickRewardView.lua
-- @Description:   文件描述
-- @Author: ZDH
-- @Date:   2024-02-01 11:08:40
-- @Copyright:   (LY) 2023 雷焰网络

module('game.shootBrick.view.ShootBrickRewardView', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("shootBrick/ShootBrickRewardView.prefab")
...
function configUI(self)
    ...
    self.m_scroller:SetItemRender(shootBrick.ShootBrickRewardItem)
...
function initViewText(self)
    self.mTextTitle.text = _TT(138701)
...
function active(self, map_id)
    super.active(self)
    GameDispatcher:addEventListener(EventName.SHOOTBRICK_RECEIVE_REWARD, self.udpateView, self)
...
function deActive(self)
    ...
    GameDispatcher:removeEventListener(EventName.SHOOTBRICK_RECEIVE_REWARD, self.udpateView, self)
end
...
function udpateView(self)
    ...
    for rewardId, rewardConfigVo in pairs(rewardConfigDic) do
        local rewardState = shootBrick.ShootBrickManager:getRewardGetState(rewardId)
    ...
    local mainActivityVo = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.ShootBrick)
...
function onGetAll(self)
    -- logAll("请求全部领取")
    GameDispatcher:dispatchEvent(EventName.ONREQ_SHOOTBRICK_GET_AWARD, self.canGetList)
end
