...
module("fashionPermit.FashionPermitItem", Class.impl("lib.component.BaseItemRender"))
--对应的ui文件
...
function active(self)
    super.active(self)
    GameDispatcher:addEventListener(EventName.UPDATE_FASHION_PERMIT_PANEL, self.updateState, self)
end

function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_FASHION_PERMIT_PANEL, self.updateState, self)
...
function onClickReciveNHandler(self)
    GameDispatcher:dispatchEvent(EventName.REQ_GAIN_FASHION_PERMIT_REWARD, { lv = self.data.lv, isSenior = 0 })
end

function onClickReciveSHandler(self)
    GameDispatcher:dispatchEvent(EventName.REQ_GAIN_FASHION_PERMIT_REWARD, { lv = self.data.lv, isSenior = 1 })
end
...
function updateState(self)
    ...
    for _, propsVo1 in ipairs(self.data:getSeniorAwardList()) do
        ...
        local isBuy = fashionPermit.FashionPermitManager:getIsBuyFashionPermit()
