-- @FileName:   RankingDupPanel.lua
-- @Description:   文件描述
-- @Author: ZDH
-- @Date:   2023-07-21 10:59:29
-- @Copyright:   (LY) 2023 雷焰网络

module('game.ranking.view.RankingDupPanel', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("ranking/RankingDupPanel.prefab")
...
function updateView(self, args, noShow)
    ...
    local starCount = ranking.RankingManager:getPassStageStar(self.m_DupConfigVo.id)
    self:clearItem()
    self.mStarItemList = {}
    for i = 1, #self.m_DupConfigVo.star_list do
        local starItem = SimpleInsItem:create(self.item_Score, self.listGroup, "RankingDupPanel_StarItem")
        ...
        starItem:getChildGO("mTextDesc"):GetComponent(ty.Text).text = string.format("<color=#%s>%s</color>", color, param:getDesc())
    end
...
function onClickClose(self)
    ...
    GameDispatcher:dispatchEvent(EventName.ONCLOSE_RANKING_DUPPANEL)
end

function onFight(self)
    local activity_id = activity.ActivityId.Ranking
