-- @FileName:   RankingStarAwardView.lua
-- @Description:   描述
-- @Author: ZDH
-- @Date:   2024-12-03 16:54:29
-- @Copyright:   (LY) 2024 锚点降临

module('game.ranking.view.RankingStarAwardView', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("ranking/RankingStarAwardView.prefab")
...
function configUI(self)
    ...
    self.m_scroller:SetItemRender(ranking.RankingStarAwardItem)
...
function active(self, map_id)
    super.active(self)
    GameDispatcher:addEventListener(EventName.RANKING_RECEIVE_REWARD, self.udpateView, self)
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.RANKING_RECEIVE_REWARD, self.udpateView, self)
end
...
function udpateView(self)
    local starCount = ranking.RankingManager:getPassAllStarCount()
    local rewardList = ranking.RankingManager:getRewardConfigVoDic()

    self.canGetList = {}
    local data = {}
    for i = 1, #rewardList do
        local isGet = ranking.RankingManager:getRewardGetState(rewardList[i].id)
    ...
    local mainActivityVo = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Ranking)
...
function onGetAll(self)
    -- logAll("请求全部领取")
    GameDispatcher:dispatchEvent(EventName.ONREQ_RANKING_GET_AWARD, self.canGetList)
end
