...
module('mole.MoleDupPanel', Class.impl(View))

-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("mole/MoleDupPanel.prefab")
...
function updateView(self, args, noShow)
    ...
    local starCount = mole.MoleManager:getDupPassStar(self.m_DupConfigVo.id)
    self:clearItem()
    self.mStarItemList = {}
    for i = 1, #self.m_DupConfigVo.starList do
        local starItem = SimpleInsItem:create(self.item_Score, self.listGroup, "MoleupPanel_StarItem")
        ...
        local param = self.m_DupConfigVo.starList[i]
        local starVo = mole.MoleManager:getStarDataById(param)
     
        starItem:getChildGO("mTextDesc"):GetComponent(ty.Text).text = string.format("<color=#%s>%s</color>", color, _TT(starVo.des))
    end
...
function onFight(self)
    local activity_id = activity.ActivityId.Mole
