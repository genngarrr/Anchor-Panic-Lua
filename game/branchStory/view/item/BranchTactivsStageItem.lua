module("branchStory.BranchTactivsStageItem", Class.impl(branchStory.BranchMainStageItem))
...
function getCanPass(self)
    return branchStory.BranchTactivsManager:isTactivsStagePass(self:getStageVo().chapterId, self:getStageVo().stageId)
end

function __updateContentView(self)
    self.mBranchStoryStageIcon:GetComponent(ty.AutoRefImage):SetImg(UrlManager:getPackPath("tacticeTrain/img_stage_item_zs.png"), false)
    ...
    elseif branchStory.BranchTactivsManager:canTactivsStageFight(stageVo.chapterId, stageId) then
        self.m_goLock:SetActive(false)
        self.mBranchStoryStageIcon:SetActive(true)
