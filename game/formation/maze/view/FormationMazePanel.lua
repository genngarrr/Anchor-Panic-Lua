...
module('fomation.FormationMazePanel', Class.impl(formation.FormationPanel))
...
function __updateView(self, cusInit)
    if (self:isLoadFinish()) then
        ...
        if (data.battleType and data.dupId) then
    ...
    if (isHasEmpty) then
            ...
            for i = 1, #tempTidList do
                if (table.indexof(uniqueTidList, tempTidList[i]) == false) then
                    ...
                    for j = #uniqueTidList, 1, -1 do
                        ...
                        local tempMazeHeroVo = maze.MazeManager:getMazeHero(self:getManager():getData().data.mazeId, tempMonsterVo.uniqueTid, maze.HERO_SOURCE_TYPE.SUPPORT)
                        if (tempMonsterVo.tid == compareMonsterVo.tid) then
                            local compareMazeHeroVo = maze.MazeManager:getMazeHero(self:getManager():getData().data.mazeId, compareMonsterVo.uniqueTid, maze.HERO_SOURCE_TYPE.SUPPORT)
            ...
            if (uniqueTidList and #uniqueTidList > 0) then    
                ...
                for i = 1, #uniqueTidList do
                    ...
                    if (not isExistSame) then
                        local mazeHeroVo = maze.MazeManager:getMazeHero(self:getManager():getData().data.mazeId, monsterTidVo.uniqueTid, maze.HERO_SOURCE_TYPE.SUPPORT)
        ...
        if (#selectTidList >= #self.m_myAllHeroTidList) then
            ...
            for i = 1, #self.m_myAllHeroIdList do
                ...
                local mazeHeroVo = maze.MazeManager:getMazeHero(self:getManager():getData().data.mazeId, heroId, maze.HERO_SOURCE_TYPE.OWN)
                if (mazeHeroVo.nowHp > 0) then
    ...
    for col_x = 1, col do
        for row_y = 1, row do
            ...
            if tileFormationHeroVo then
                local mazeHeroVo = self:getMazeHeroVo(tileFormationHeroVo)
                if (mazeHeroVo.nowHp <= 0) then
                    hasZeroHp = true
                    break
                end
        ...
        for row_y = 1, row do
            ...
            if tileFormationHeroVo then
                ...
                if mazeHeroVo.nowHp > 0 then
                    ...
                    -- item:setText("mTxtCont", nil, _TT(29120, mazeHeroVo.nowHp, mazeHeroVo.maxHp))
                    ...
                    -- item:setText("mTxtCont", nil, _TT(29121, mazeHeroVo.nowHp, mazeHeroVo.maxHp))
...
function getMazeHeroVo(self, formationHeroVo)
    local mazeHeroVo = nil
    if (formationHeroVo:getIsMonster()) then
        mazeHeroVo = maze.MazeManager:getMazeHero(self:getManager():getData().data.mazeId, formationHeroVo.heroId, maze.HERO_SOURCE_TYPE.SUPPORT)
    else
        mazeHeroVo = maze.MazeManager:getMazeHero(self:getManager():getData().data.mazeId, formationHeroVo.heroId, maze.HERO_SOURCE_TYPE.OWN)
    end
    return mazeHeroVo
end
