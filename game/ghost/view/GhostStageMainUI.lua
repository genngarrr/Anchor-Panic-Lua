...
module('ghost.GhostStageMainUI', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("ghost/GhostStageMainUI.prefab")
...
function ctor(self)
    ...
    self:setBg("bg_01.jpg",true,"ghost")

    self:setTxtTitle(_TT(151221))
    self:setUICode(LinkCode.Ghost)
end
...
function onClickFuncTipsHandler(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_GHOST_TIPS_VIEW)
...
function onClickTask(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_GHOST_TASK_PANEL)
end

function onClickRank(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_GHOST_RANK_PANEL)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_GHOST_STAGE_PANEL,self.showPanel,self)
    self:showPanel()
end
...
function showPanel(self)
    ...
    if area_id then
        local areaConfig = ghost.GhostManager:getAreaConfig(area_id)
        ...
        if lastArea > 0 then
            if not ghost.GhostManager:getAreaPassState(lastArea) then
                area_id = 1
            end
        end
    else
        local areaConfigDic = ghost.GhostManager:getAreaConfigDic()
        for i = 1, #areaConfigDic do
            local timeOpen = areaConfigDic[i]:isOpen()
            local lastOpen = true
            if i > 1 then
                lastOpen = ghost.GhostManager:getAreaPassState(areaConfigDic[i - 1].tid)
            end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_GHOST_STAGE_PANEL,self.showPanel,self)
...
function close(self)
    GameDispatcher:dispatchEvent(EventName.CLOSE_GHOST_DUP_PANEL)
...
function addAllEventListener(self)
    ...
    GameDispatcher:addEventListener(EventName.GHOST_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:addEventListener(EventName.UPDATE_GHOST_STAGE_PANEL, self.onDataRefresh, self)
end
...
function removeAllEventListener(self)
    ...
    GameDispatcher:removeEventListener(EventName.GHOST_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:removeEventListener(EventName.UPDATE_GHOST_STAGE_PANEL, self.onDataRefresh, self)
    

end

function onClickReward(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_GHOST_STAR_REWARD_PANEL)
end
...
function onSelectMapTab(self, area_id)
    ...
    for i = 1, length do
        ...
        local configVo = ghost.GhostManager:getDupConfig(dup_id)
        ...
        --local star_count = ghost.GhostManager:getDupPassStar(dup_id)
        for i = 1, 3 do
            dupItem:getChildGO("mStar_" .. i):SetActive(false)
        end

        local isPass = ghost.GhostManager:getDupPassState(dup_id)
        ...
        if configVo.pre_id ~= 0 then
            lastPass = ghost.GhostManager:getDupPassState(configVo.pre_id)
        end
        ...
        dupItem:addUIEvent("mRayPoint", function ()
            ...
            StorageUtil:saveBool1(gstor.GHOST_DUPNEWOPENSTR .. dup_id, true)
...
function onSelectDupItem(self, dupConfigVo)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_GHOST_DUP_PANEL)
    GameDispatcher:dispatchEvent(EventName.OPEN_GHOST_DUP_PANEL, dupConfigVo)
end
...
function refreshAreaRedPoint(self)
    local areaConfig = ghost.GhostManager:getAreaConfigDic()
    for areaId, areaConfigVo in pairs(areaConfig) do
        local tabItem = self.m_tabItemList[areaId]
        if tabItem then
            if ghost.GhostManager:getAreaShowRed(areaConfigVo) then
...
function refreshDupRedPoint(self)
    for i = 1, #self.m_areaConfig.stage_list do
        ...
        if dupItem then
            if ghost.GhostManager:getDupShowRed(dup_id) then
...
function refreshStarAwardRedPoint(self)
    if ghost.GhostManager:getTaskRed() then
        RedPointManager:add(self.mBtnTask.transform, nil, 30,30)
    else
        RedPointManager:remove(self.mBtnTask.transform)
    end
    --self.ImEffect:SetActive(ghost.GhostManager:getStarAwardRedState())
end
