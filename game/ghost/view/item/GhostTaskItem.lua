--[[ 
-----------------------------------------------------
@filename       : GhostTaskItem
@Description    : 活动玩法任务Item
@date           : 2023/5/24 15:00
@Author         : Tonn
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------]]
module("ghost.GhostTaskItem ", Class.impl("lib.component.BaseItemRender"))
...
function setData(self, param)
    ...
    if self.data then
        ...
        local received = ghost.GhostManager:checkTaskAwardReceived(self.data.id)
        self:getChildGO("mGroupReceived"):SetActive(received)

        local taskMsgVo =  ghost.GhostManager:getTaskMsgVo(self.data.id)
