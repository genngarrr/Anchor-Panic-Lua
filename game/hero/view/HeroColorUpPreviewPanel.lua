...
module('hero.HeroColorUpPreviewPanel', Class.impl(View))
UIRes = UrlManager:getUIPrefabPath('hero/HeroColorUpPreviewPanel.prefab')
...
function __recyAllItem(self)
    ...
    if (self.m_skillItemList) then
        for i = 1, #self.m_skillItemList do
            local item = self.m_skillItemList[i]
            item:poolRecover()
        end
    end
...
function __updatePrevewView(self)
	-- 更新普通的属性展示
	local nextAttrDic = self.m_heroVo:getColorUpPreviewAttrDic()
	if(nextAttrDic)then
		local index = 0
		...
		for i = 1, len do
			...
			if(nextAttrDic[attrVo.key] and attrVo.value ~= nextAttrDic[attrVo.key])then
				index = index + 1
				local item = hero.HeroColorUpListItem:poolGet()
...
function __updatePrevewFullView(self)
	local curColorUpConfigVo = hero.HeroColorUpManager:getHeroColorUpConfigVo(self.m_heroVo.tid, self.m_heroVo.color)
	-- local nextColorUpConfigVo = hero.HeroColorUpManager:getHeroColorUpConfigVo(self.m_heroVo.tid, self.m_heroVo.color + 1)
	-- 更新普通的属性展示
	local attrDic = self.m_heroVo.attrDic
	if(attrDic)then
		local index = 0
		local len = #curColorUpConfigVo.keyList
		for i = 1, len do
			local key = curColorUpConfigVo.keyList[i]
			if(attrDic[key])then
				index = index + 1
				local item = hero.HeroColorUpListItem:poolGet()
	...
	local skillIlist = self.m_heroVo.baseSkillIdList
    for i = 1, #skillIlist do
        local skillId = skillIlist[i]
        ...
        if (skillType ~= fight.FightDef.SKILL_TYPE_NORMAL_ATTACK) then
			local item = hero.HeroColorUpSkillItem:poolGet()
			item:setData(skillId, self.m_heroVo.id, false)
			item:setParent(self.m_groupSkillFull)
			table.insert(self.m_skillItemList, item)
		end
	end

    -- local colorDic = hero.HeroColorUpManager:getHeroColorUpDic(self.m_heroVo.tid)
    -- for color, vo in pairs(colorDic) do
    --     local item = hero.HeroColorUpSkillItem:poolGet()
    --     item:setData(color, self.m_heroVo.id, false)
    --     item:setParent(self.m_groupSkillFull)
    --     table.insert(self.m_skillItemList, item)
    -- end
end
...
function __updateUnlockSkillView(self)
	local nextColorUpConfigVo = hero.HeroColorUpManager:getHeroColorUpConfigVo(self.m_heroVo.tid, self.m_heroVo.color + 1)
	if(nextColorUpConfigVo)then
		if(nextColorUpConfigVo.skillId > 0)then
			local skillVo = fight.SkillManager:getSkillRo(nextColorUpConfigVo.skillId)
			if(skillVo)then
...
function __onHerPreviewAttrUpdateHandler(self, args)
	...
	if(previewType == hero.PreviewAttrType.COLOR_UP)then
...
function __onSkillTipHandler(self)
	-- local curColorUpConfigVo = hero.HeroColorUpManager:getHeroColorUpConfigVo(self.m_heroVo.tid, self.m_heroVo.color + 1)
	local nextColorUpConfigVo = hero.HeroColorUpManager:getHeroColorUpConfigVo(self.m_heroVo.tid, self.m_heroVo.color + 1)
	if(nextColorUpConfigVo)then
		TipsFactory:skillTips(nil, nextColorUpConfigVo.skillId, self.m_heroVo)
	end
end
