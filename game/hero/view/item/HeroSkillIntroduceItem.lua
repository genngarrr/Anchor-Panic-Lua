--[[ 
-----------------------------------------------------
@filename       : HeroSkillIntroduceItem
@Description    : 招募预览技能item
@date           : 2023-3-27 15:19:00
@Author         : lyx
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------
]]
module("hero.HeroSkillIntroduceItem", Class.impl(SimpleInsItem))
...
function updateContent(self)
    ...
    if (skillType == fight.FightDef.SKILL_TYPE_PASSIVE_SKILL) then
        ...
        local skillLv = "1" --heroVo:getActivePassiveSkill(skillVo:getRefID()) + heroVo:getExtraLv(skillVo:getRefID())
        mTxtLv.text = skillLv
        mTxtSkillLv.text = "Lv." .. skillLv
    else
        self.m_childGos["mImgItemBg"]:SetActive(true)
        local skillLv = "1" --heroVo:getActiveSkill(skillVo:getRefID()) + heroVo:getExtraLv(skillVo:getRefID())
        mTxtLv.text = skillLv
        mTxtSkillLv.text = "Lv." .. skillLv
    end
    --锁定状态
    ...
    if (#skillVo:getLocation() >= 2) then
        mImgSkillDefine.color = gs.ColorUtil.GetColor(skillVo:getLocation()[1])
        mTxtSkillDefine.text = _TT(skillVo:getLocation()[2])
        mTxtSkillDefineRight.text = _TT(skillVo:getLocation()[2])
    else
    ...
    if (self.mItemType == 1) then
        ...
        -- if self.m_data.isUnlock == 1 then
        --     -- local canUp = false
        --     -- local extraLv = 0 --heroVo:getExtraLv(skillVo:getRefID())
        --     -- local maxSkillLvl = sysParam.SysParamManager:getValue(SysParamType.SKILLCEILING) - 1
        ...
        --     -- end
        --     -- if canUp then
        --     --     self.m_childGos["mImgSkillUp"]:SetActive((true and (self.mIsShowLeftUp == false)))
        --     --     self.m_childGos["mImgSkillUpLeft"]:SetActive(self.mIsShowLeftUp)
        --     -- else
        --     self.m_childGos["mImgSkillUp"]:SetActive(false)
