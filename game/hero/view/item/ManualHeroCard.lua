module("hero.ManualHeroCard", Class.impl(BaseComponent))

-- UI预制体名
UIRes = UrlManager:getUIPrefabPath("hero/item/ManualHeroCard.prefab")
...
function __updateCustomView(self)
    ...
    self:getChildGO("mTxtAct"):GetComponent(ty.Text).text = _TT(10000404)
...
function updateManualInfo(self)
    local isAct = manual.ManualHeroManager:getHeroActByTid(self:getData().tid)
    local isObtain = hero.HeroManager:getIsObtain(self:getData().tid)
    self:getChildGO("mGroupNot"):SetActive(isObtain == false or isAct == false)
    ...
    self:getChildGO("mTxtAttr"):GetComponent(ty.Text).text = "+" .. self:getData().firstActivateAttr[2]/100 .. "%"
end
