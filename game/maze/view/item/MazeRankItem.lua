module("maze.MazeRankItem", Class.impl("lib.component.BaseItemRender"))

function onInit(self, go)
    super.onInit(self, go)

    self.m_imgBg = self:getChildGO("ItemImgBg"):GetComponent(ty.Image)
    self.m_iconRankGo = self:getChildGO("ItemIconRank")
    ...
    self.mTxtName = self:getChildGO("ItemTextName"):GetComponent(ty.Text)
    self.m_textScore = self:getChildGO("ItemTextScore"):GetComponent(ty.Text)

    self.m_headNode = self:getChildTrans("ItemHeadGridNode")
