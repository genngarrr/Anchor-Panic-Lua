module("maze.MazeRankPanel", Class.impl(TabSubView))

UIRes = UrlManager:getUIPrefabPath("maze/MazeRankPanel.prefab")
...
function configUI(self)
    ...
    self.m_scroller:SetItemRender(maze.MazeRankItem)
...
function initViewText(self)
    ...
    self.m_textTileScore.text = "通关时长"--_TT(159)
end

function active(self)
    super.active(self)

    maze.MazeManager:addEventListener(maze.MazeManager.EVENT_MAZE_RANK_UPDATE, self.__updateView, self)
    -- 请求排行榜面板内容
    GameDispatcher:dispatchEvent(EventName.REQ_MAZE_RANK_INFO)
    self:__updateView()
end
...
function deActive(self)
    ...
    maze.MazeManager:removeEventListener(maze.MazeManager.EVENT_MAZE_RANK_UPDATE, self.__updateView, self)
end

function __updateView(self, cusIsInit)
    if not maze.MazeManager.myRank then
        return
    end
    local list = maze.MazeManager.rankList
    ...
    self.m_textScore.text = TimeUtil.getMSByTime(maze.MazeManager.myPassTime)
end
