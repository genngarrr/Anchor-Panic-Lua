module("permit.PermitBuyView", Class.impl(View))

UIRes = UrlManager:getUIPrefabPath("permit/PermitBuyView.prefab")
...
function ctor(self)
    ...
    self:setTxtTitle(_TT(98103))--通行证
end
-- 初始化数据
...
function configUI(self)
    ...
    self.mTxtLvDes = self:getChildGO("mTxtLvDes"):GetComponent(ty.Text)
    ...
    self.mTxtBuylvDes = self:getChildGO("mTxtBuylvDes"):GetComponent(ty.Text)
    ...
    self.mNumberStepper:Init(1, 1, 1, (#permit.PermitManager:getPermitList() - permit.PermitManager:getPermitedLv()), self.onStepChange, self)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_PERMIT_LV, self.onCloseView, self)
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_PERMIT_LV, self.onCloseView, self)
...
function initViewText(self)
    self.mTxtLvDes.text=_TT(1000081023)--"許可證等級"
    self.mTxtBuylvDes.text=_TT(98103)..":"
    self.mTxtTitle.text=_TT(1000081024)--"可獲得獎勵預覽:"
...
function updateView(self)
    self.mTxtCurLv.text = permit.PermitManager:getPermitedLv()
    self.mCurLv = permit.PermitManager:getPermitedLv()
    self:updatePrice()
    self.mNumberStepper.MaxCount = #permit.PermitManager:getPermitList() - self.mCurLv
end
...
function onStepChange(self, cusCount, cusType)
    if cusType == 1 then
        -- '最大值'
        if self.mNumberStepper.MaxCount >= (#permit.PermitManager:getPermitList() - self.mCurLv) then
...
function onClickHandler(self)
    ...
    GameDispatcher:dispatchEvent(EventName.REQ_PERMIT_BUY_LV, { buyLv = self.mNumberStepper.CurrCount })
end

function updatePrice(self)
    self:closeAllProps()
    self.mNeedMoney = self.mNumberStepper.CurrCount * sysParam.SysParamManager:getValue(SysParamType.UP_NEED_DIAMOND)
    ...
    local list = permit.PermitManager:getPermitLvStageAwardList(needBuyLv)
