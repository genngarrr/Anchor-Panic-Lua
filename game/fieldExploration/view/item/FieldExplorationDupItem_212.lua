...
function setData(self, dup_id, lastDup_id)
    ...
    local showRed = self.isCanPlay and not StorageUtil:getBool1(FieldExplorationConst.DupNewOpenStr .. dup_id)
    if showRed and self.isCanPlay then
        local isPass = fieldExploration.FieldExplorationManager:getPlayerDupRecord(dup_id) ~= 0
        showRed = not isPass
    end
    self:getChildGO("mRedImg"):SetActive(showRed)
