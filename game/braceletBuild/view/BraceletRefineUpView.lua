...
module('braceletBuild.BraceletRefineUpView', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("braceletBuild/BraceletRefineUpView.prefab")
...
function configUI(self)
    ...
    self.mLyScroller:SetItemRender(braceletBuild.BraceletRemakeMaterialItem)
...
function active(self, args)
    ...
    self.mHeroId = braceletBuild.BraceletBuildManager.nowHeroId
    braceletBuild.BraceletBuildManager:addEventListener(braceletBuild.BraceletBuildManager.BRACELET_STRENGTHEN_MATERIAL, self.__onRemakeMaterialSelectHandler, self)
    GameDispatcher:addEventListener(EventName.UPDATE_EQUIP_REMAKE, self.__onUpdateRemakeHandler, self)
    ...
    if (self.mVisibleCallFun) then
        self.mVisibleCallFun(true, self.m_equipMaterialVo)
    end
end
...
function deActive(self)
    super.deActive(self)
    braceletBuild.BraceletBuildManager:removeEventListener(braceletBuild.BraceletBuildManager.BRACELET_STRENGTHEN_MATERIAL, self.__onRemakeMaterialSelectHandler, self)

    GameDispatcher:removeEventListener(EventName.UPDATE_EQUIP_REMAKE, self.__onUpdateRemakeHandler, self)
    GameDispatcher:removeEventListener(EventName.CHANGE_BRACELETS, self.close, self)
    role.RoleManager:getRoleVo():removeEventListener(role.RoleVo.CHANGE_PLAYER_MONEY, self.__updateView, self)
    if (self.mVisibleCallFun) then
        self.mVisibleCallFun(false, self.m_equipMaterialVo)
    end
end
...
function __onClickConfirmHandler(self)
    ...
    --         -- local isNotRemind = remind.RemindManager:isTodayNotRemain(RemindConst.EQUIP_REMAKE)
    --         -- if (isNotRemind) then
    --         --     onReqRemakeHandler()
    --         -- else
...
function setData(self, args)
    ...
    self.m_equipMaterialVo = braceletBuild.BraceletBuildManager:getSelectRefineVo()
...
function updateStrengthenView(self, isInit)
    ...
    local list = braceletBuild.BraceletBuildManager:getIdenticalBraceletList(self.mEquipVo.tid, equipId)
    -- if isInit then
    ...
    equipBuild.EquipRemakeManager:sortHandler(list, self.m_selectSortType, self.m_isDescending)

    for i = 1, #list do
        local propsScrollVo = list[i]
        propsScrollVo:setSelect(false)
        local propsVo = propsScrollVo:getDataVo()
        if (self.m_equipMaterialVo and self.m_equipMaterialVo.id == propsVo.id) then
...
function __onRemakeMaterialSelectHandler(self, args)
    if args then
        self.m_equipMaterialVo = args:getDataVo()
    else
        self.m_equipMaterialVo = nil
    end 
