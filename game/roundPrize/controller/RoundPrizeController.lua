...
module("game.roundPrize.RoundPrizeController", Class.impl(Controller))
...
function listNotification(self)
    GameDispatcher:addEventListener(EventName.ROUNDPRIZE_OPENRULEPANEL, self.onOpenRoundPrizeRulePanelHandler, self)
    GameDispatcher:addEventListener(EventName.ROUNDPRIZE_OPENSHOPPANEL, self.onOpenRoundPrizeShopPanelHandler, self)
    GameDispatcher:addEventListener(EventName.ROUNDPRIZE_CLOSE_SHOWAWARDPANEL, self.onShowRewardClose, self)

    GameDispatcher:addEventListener(EventName.ROUNDPRIZE_ONREQPRIZE, self.onReqPrize, self)
    GameDispatcher:addEventListener(EventName.ROUNDPRIZE_ONREQGETREWARD, self.onReqGetReward, self)
...
function registerMsgHandler(self)
    return {
        --- *s2c* 新年轮盘面板 24470
        SC_NEW_YEAR_TURNTABLE_PANEL = self.onReceiveMainPanMsg,
        --- *s2c* 新年轮盘抽奖 24472
        SC_NEW_YEAR_TURNTABLE_LOTTERY = self.onReceivePrizeHandler,
        --- *s2c* 领取累抽奖励 24474
        SC_NEW_YEAR_TURNTABLE_GAIN_ACC_AWARD = self.onReceiveGetRewardHandler,
    }
end
...
function onShowRewardClose(self)
    if self.m_PrizeData then
        ...
        GameDispatcher:dispatchEvent(EventName.ROUNDPRIZE_ONRECEIVEMAINPANMSG)
    end
end
...
function onReceiveMainPanMsg(self, msg)
    ...
    roundPrize.RoundPrizeManager:parseMainInfo(msg)
    GameDispatcher:dispatchEvent(EventName.ROUNDPRIZE_ONRECEIVEMAINPANMSG)
    GameDispatcher:dispatchEvent(EventName.UPDATE_CELEBRATION_RED_STATE)
    GameDispatcher:dispatchEvent(EventName.UPDATE_ACTIVITY_RED)
end
...
function onReceivePrizeHandler(self, msg)
    ...
    GameDispatcher:dispatchEvent(EventName.ROUNDPRIZE_ONRECEIVEPRIZEHANDLER, msg)
...
function onReqPrize(self, times)
    ...
    SOCKET_SEND(Protocol.CS_NEW_YEAR_TURNTABLE_LOTTERY, {times = times}, Protocol.SC_NEW_YEAR_TURNTABLE_LOTTERY)
end
...
function onReqGetReward(self, times)
    ...
    SOCKET_SEND(Protocol.CS_NEW_YEAR_TURNTABLE_GAIN_ACC_AWARD, {times = times}, Protocol.SC_NEW_YEAR_TURNTABLE_GAIN_ACC_AWARD)
end
...
function onOpenRoundPrizeRulePanelHandler(self, args)
    if self.mRoundPrizeRulePanel == nil then
        self.mRoundPrizeRulePanel = UI.new(roundPrize.RoundPrizeRulePanel)
...
function onOpenRoundPrizeShopPanelHandler(self, args)
    if self.mRoundPrizeShopPanel == nil then
        self.mRoundPrizeShopPanel = UI.new(roundPrize.RoundPrizeShopPanel)
