...
function createPropsGrid(self, list)
    ...
    local create_limit = #self.mlist
    if create_limit >= showAwardNum then
        self.mImgNext:SetActive(true)
    else
        self.mImgNext:SetActive(false)
    end

    for i = 1, create_limit do
    ...
    local create_index = create_limit
    self:addTimer(0.25, create_limit, function ()
        local grid = self.mItemList[create_index]
        local propsVo = self.mlist[create_limit - create_index + 1]
        grid:setData(nil, propsVo)

        if create_index <= 1 then
            self:getChildGO("Viewport"):GetComponent(ty.RaycastEmptyEvent).enabled = create_limit > 8
