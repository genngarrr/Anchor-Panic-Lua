module("dup.DupImpliedAwardPanel", Class.impl(TabSubView))

UIRes = UrlManager:getUIPrefabPath("dupImplied/DupImpliedAwardPanel.prefab")
...
function configUI(self)
    ...
    self.m_scroller:SetItemRender(dup.DupImpliedAwardItem)
...
function onTimer(self)
    ...
    -- self.m_textTime.text = _TT(29142, TimeUtil.getFormatTimeBySeconds_1(endTime))

    local currentTime = GameManager:getClientTime()
    local reamainTime = GameManager:getWeekResetTime() - currentTime
    self.m_textTime.text = _TT(29142,TimeUtil.getFormatTimeBySeconds_1(reamainTime))
end

function updateView(self, cusIsInit)
    -- local type = self.m_tabBar:getType()
    local list = dup.DupImpliedManager:getRewardList()
    local list2 = {}
    for i = 2, #list - 1 do
    ...
    self.m_textAwardType.text = _TT(29143)
