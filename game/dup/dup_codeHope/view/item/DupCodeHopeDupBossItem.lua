--[[ 
-----------------------------------------------------
@filename       : DupCodeHopeDupItem_BOSS
...
module('game.dup.dup_codeHope.view.item.DupCodeHopeDupBossItem', Class.impl(BaseReuseItem))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("dupCodeHope/DupCodeHopeDupItem_BOSS.prefab")
...
function setData(self, cusParent, cusDupId)
    ...
    if self.data.stageType == 2 then
        -- 特殊关卡
        --self.mBtnBg:GetComponent(ty.AutoRefImage):SetImg(UrlManager:getPackPath("dupCodeHope/dupCodeHope_49.png"))
    else
        --self.mBtnBg:GetComponent(ty.AutoRefImage):SetImg(UrlManager:getPackPath("dupCodeHope/dupCodeHope_14.png"))
    end
...
--     self.selectTween = TweenFactory:move2LPosY(self.mImgSelect, 76.0, 0.3, nil, function()
--         self:__tweenArrow1()
--     end)
-- end
