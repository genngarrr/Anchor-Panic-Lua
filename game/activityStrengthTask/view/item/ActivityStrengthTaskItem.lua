...
module("game.activityStrengthTask.view.item.ActivityStrengthTaskItem", Class.impl('lib.component.BaseItemRender'))
--构造函数
function ctor(self)
    super.ctor(self)
    self.mAwardItemList = {}
...
function updateDataView(self)
    ...
    local linkId = self.data:getUICode()
    self.mImgIng:SetActive(self.data:getState()==activityStrengthTask.ActivityStrengthTaskConst.CelebrationTaskState.Nomal and linkId == 0)
    self.mBtnGoto:SetActive(self.data:getState()==activityStrengthTask.ActivityStrengthTaskConst.CelebrationTaskState.Nomal and linkId ~= 0)
    self.mBtnRecive:SetActive(self.data:getState()==activityStrengthTask.ActivityStrengthTaskConst.CelebrationTaskState.Recive)
    self.mImgRecived:SetActive(self.data:getState()==activityStrengthTask.ActivityStrengthTaskConst.CelebrationTaskState.Recived)
...
function onClickGetHandler(self)
    GameDispatcher:dispatchEvent(EventName.REQ_RECIVE_ACTIVITY_STRENGTH_TASK_TASK_AWARD, { self.data.id })
end
