...
function configUI(self)
    ...
    self.mImgJob = self:getChildGO("mImgJob"):GetComponent(ty.Image)
...
function initViewText(self)
    local paramsData = sysParam.SysParamManager:getValue(SysParamType.CELEBRATION_TASK_FASHION_INFO)
    local heroTid = paramsData[1]
    local heroConfigVo = hero.HeroManager:getHeroConfigVo(heroTid)
    self.mImgJob:SetImg(UrlManager:getHeroJobSmallIconUrl(heroConfigVo.professionType), false)
    self.mTxtTitle.text = _TT(98107)--"动态立绘"
    self.mTxtHeroName.text = _TT(121198)
    self.mTxtFashionName.text = _TT(121199)
...
function updateList(self,isInit)
    ...
    gs.TransQuick:SizeDelta02(self.mLyScroller.gameObject:GetComponent(ty.RectTransform),
     #Celebration.CelebrationManager:getCanReciveListByDay(self.mCurDay)>0 and  365 or 457)
