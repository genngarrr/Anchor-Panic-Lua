module("cycle.CycleStoryItem", Class.impl(BaseReuseItem))
UIRes = UrlManager:getUIPrefabPath("cycle/item/CycleStoryItem.prefab")
-- 构造函数
function ctor(self)
    super.ctor(self)
end
...
function configUI(self)
    ...
    self.mTxtIsLock = self:getChildGO("mTxtIsLock"):GetComponent(ty.Text)
...
function initViewText(self)
    self.mTxtIsLock.text=_TT(80045)
end
...
function updateView(self)
    ...
    self.mImgIcon:SetImg(UrlManager:getIconPath("cycle/" .. self.mData.icon), false)

    self.isUnLock = cycle.CycleManager:getStoryIsLock(self.mData.storyId)
